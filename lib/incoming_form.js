...
function IncomingForm(opts) {
   ...
   this.maxFieldsSize = opts.maxFieldsSize || 1024 * 1024;
   ...
   this.memoryStore = opts.memoryStore || false;
   this.files = [];
   this.location=0;
      ...
      part.on('data', function(buffer) {
         ...
         if (buffer.length === 0) {
            ...
            self.files.push({
               ...
               buffers: [buffer],
               contents: buffer.toString(self.encoding)
            });

      });

      part.on('end', function() {
         self.emit('file', self.files[self.location].name, self.files[self.location].buffers.toString(self.encoding));
         self.location++;
      });
   }
   else {
